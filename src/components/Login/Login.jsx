...
const Login = (props) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setError(true);
            if (err.response.data != null && err.response.data === "Incorrect username or password") {
    ...
    return (
        <form method="POST" className="sign-in-form" onSubmit={handleLogin}>
            ...
            <div className={`login-error ${error === true ? "active-error" : ""}`} id="show-login-error">
                <FaCircleXmark className='icon' />
                <p className="msg-sign-in">{msgError}</p>
            </div>
            <div className="input-field">
                <FaUser className='icon' />
                <input type="text" placeholder={t('username')} name="username" id="sign-in-username" required autoFocus autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} />
            </div>
            <div className="input-field">
                <FaLock className='icon' />
                <input type="password" placeholder={t('password')} name="password" id="sign-in-password" required autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} />
            </div>
            {/* Remember Me */}
